...
如上所述，代码转换器封装了三个值：编码解码器，eol 风格，和错误处理模式。本节介绍了创建和操作代码转换器的过程，以及代码转换器封装的值。
...
returns: 封装了编码解码器，eol 风格，和错误处理模式的代码转换器
...
原生的代码转换器是依赖于实现的，且可能根据平台或区域设置有所变化。
...
每个文件打开操作接受一个路径实参，命名了要打开的文件。它必须是一个字符串或其它某些平台相关的命名了一个文件的值。
